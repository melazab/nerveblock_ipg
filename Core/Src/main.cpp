...
int main( void )
{
    ...
    ITM->TER          = 1;             // Enable ITM port 0
    ITM->TCR  = ITM_TCR_ITMENA_Msk | ITM_TCR_SYNCENA_Msk | ITM_TCR_DWTENA_Msk | ITM_TCR_SWOENA_Msk;
    DWT->CTRL = 0x400003FE;
    ...
    gpio_initialization();
    // HAL_GPIO_WritePin( GPIOD, PWR_LED_Pin, GPIO_PIN_SET );    // Power LED
    ...
    //nfc_rx_falling_edge();
    /* USER CODE BEGIN 2 */
    // HAL_GPIO_WritePin( GPIOB, IMP_EN_Pin, GPIO_PIN_SET );
    ...
    Saturn1_spi::SATURN_DIG_STARTUP();
    Saturn1_spi::SATURN_BBC_STARTUP_R1();
    Saturn1_spi::DEFAULT_STIM();
    Saturn1_spi::Calibration();

    Saturn2_spi::SATURN_DIG_STARTUP();
    Saturn2_spi::SATURN_BBC_STARTUP_R1();
    Saturn2_spi::DEFAULT_STIM();
...
void HAL_TIM_PeriodElapsedCallback( TIM_HandleTypeDef* htim )
{
    ...
    if ( htim->Instance == TIM17 )
    {
        ...
        Saturn1_spi::CurrentTick( tick );
        Saturn2_spi::CurrentTick( tick );

        // if ( HAL_GPIO_ReadPin( GPIOC, HF_STIM_PROG_Pin )
        //      || HAL_GPIO_ReadPin( GPIOA, LF_STIM_PROG_Pin ) == 1 )
        // {
        //     HAL_GPIO_WritePin( GPIOD, STIM_LED_Pin, GPIO_PIN_SET );
        // }
        // else
        // {
        //     HAL_GPIO_WritePin( GPIOD, STIM_LED_Pin, GPIO_PIN_RESET );
        // }
