...
#define STD_TASK_STACK_SIZE  1024
#define TX_APP_MEM_POOL_SIZE 8224
...
#define TEST_STACK_BYTE_POOL_SIZE 1024
#define TASK_STACK_BYTE_POOL_SIZE 2048


#define CMD_PARSER_QUEUE_SIZE   256
#define CMD_RESPONSE_QUEUE_SIZE 256
#define QUEUE_BYTE_POOL_SIZE    ( CMD_PARSER_QUEUE_SIZE + CMD_RESPONSE_QUEUE_SIZE )

#if ( ( TASK_STACK_BYTE_POOL_SIZE + QUEUE_BYTE_POOL_SIZE ) > TX_APP_MEM_POOL_SIZE )
#    error ThreadX Memory Byte Pool not large enough for all Tasks and Queues
#endif
...
uint8_t test_stack[ TEST_STACK_BYTE_POOL_SIZE ];    // these sizes could be the size of one task
                                                    // plus a little more
uint8_t thread1_stack[ TASK_STACK_BYTE_POOL_SIZE ];
uint8_t thread2_stack[ TASK_STACK_BYTE_POOL_SIZE ];
uint8_t thread3_stack[ TASK_STACK_BYTE_POOL_SIZE ];
uint8_t thread4_stack[ TASK_STACK_BYTE_POOL_SIZE ];
uint8_t thread5_stack[ TASK_STACK_BYTE_POOL_SIZE ];
uint8_t queue_mem[ QUEUE_BYTE_POOL_SIZE ];
static TX_THREAD threadByteInParser;
static TX_THREAD threadSerailPortByteIn;
static TX_THREAD threadUSBByteIn;
static TX_THREAD threadBLEByteIn;
static TX_THREAD threadRampStim;
static TX_THREAD threadManager;
static TX_THREAD battery_charging;
TX_THREAD thread_ptr;
static UINT stop_stim_thread = TX_TRUE;

static TX_QUEUE queueCmdParser;
...
static void threadManagerEntry( ULONG thread_input );
...
void tx_application_define( void* first_unused_memory )
{
    ...
    VOID* memory_ptr;
    if ( tx_byte_pool_create( &tx_app_byte_pool,
                              "Tx App memory pool",
                              tx_byte_pool_buffer,
    ...
    else
    {
        ...
        tx_queue_create( &queueCmdParser, "CmdParserQueue", 1, queue_mem, CMD_PARSER_QUEUE_SIZE );

        crProtSetCmdParserQueue( &queueCmdParser );

        tx_thread_create( &threadByteInParser,
                          "CmdParserRecvByte",
                          crProtParserThreadEntry,
                          0,
                          thread1_stack,
                          STD_TASK_STACK_SIZE,
                          15,
        ...
        tx_thread_create( &threadUSBByteIn,
                          "DebugSerialByteIn",
                          usbByteInThreadEntry,
                          0,
                          thread2_stack,
                          STD_TASK_STACK_SIZE,
                          25,
        ...
        tx_thread_create( &threadManager,
                          "Thread Manager",
                          threadManagerEntry,
                          0,
                          thread4_stack,
        ...
        UINT status;
        status = tx_thread_create( &battery_charging,
                          "Battery Charging",
                          batt_charging_entry,
                          0,
                          thread5_stack,
        ...
        //
        //	  tx_thread_create( &threadBLEByteIn,
        //	  			  "DebugBLEByteIn",
        //	  			  BLEByteInThreadEntry,//usbByteInThreadEntry,
        //	  			  0,
        //	  			  thread3_stack,
        //	  			  STD_TASK_STACK_SIZE,
        //	  			  7,
...
static void threadManagerEntry( ULONG thread_input )
{
    while ( 1 )
    {
        if ( stop_stim_thread == TX_TRUE )
        {
            UINT status, thread_state;

            status = tx_thread_info_get( &threadRampStim,
                                         TX_NULL,
                                         &thread_state,
                                         TX_NULL,
                                         TX_NULL,
            ...
            if ( status == TX_SUCCESS )
            {
                if ( thread_state != TX_COMPLETED && thread_state != TX_TERMINATED )
                {
                    // Terminate the thread
                    status = tx_thread_terminate( &threadRampStim );
                }

                status           = tx_thread_delete( &threadRampStim );
                stop_stim_thread = TX_FALSE;
            }
        }
...
UINT start_stim_control( void )
{
    UINT status;

    stop_stim_thread = TX_FALSE;

    // Create the stimulation control thread
    status = tx_thread_create( &threadRampStim,
                               "RampStimulation",
                               rampStimThreadEntry,
                               0,
                               thread3_stack,
...
UINT stop_stim_control( void )
{
    stop_stim_thread = TX_TRUE;

    return TX_SUCCESS;
}

void pause_ramp( void )
{
    tx_thread_suspend( &threadRampStim );
}

void set_ramp_stim_flag( UINT state )
{
    stop_stim_thread = state;
}

UINT get_ramp_stim_flag( void )
{
    return stop_stim_thread;
}
...
static void Reset_IPCC( void )
{
    ...
    LL_C1_IPCC_DisableTransmitChannel( IPCC,
                                       LL_IPCC_CHANNEL_1 | LL_IPCC_CHANNEL_2 | LL_IPCC_CHANNEL_3
                                           | LL_IPCC_CHANNEL_4 | LL_IPCC_CHANNEL_5
                                           | LL_IPCC_CHANNEL_6 );
    ...
    switch ( p_sys_event->subevtcode )
    {
        ...
        case SHCI_SUB_EVT_BLE_NVM_RAM_UPDATE:
            ...
            APP_DBG_MSG(
                ...
                (unsigned long)( ( (SHCI_C2_BleNvmRamUpdate_Evt_t*)p_sys_event->payload )
                                     ->StartAddress ),
        ...
        case SHCI_SUB_EVT_NVM_START_WRITE:
            APP_DBG_MSG( "==>> SHCI_SUB_EVT_NVM_START_WRITE : NumberOfWords = %ld\n",
                         (unsigned long)( (SHCI_C2_NvmStartWrite_Evt_t*)p_sys_event->payload )
                             ->NumberOfWords );
            break;
        ...
        case SHCI_SUB_EVT_NVM_START_ERASE:
            APP_DBG_MSG( "==>>SHCI_SUB_EVT_NVM_START_ERASE : NumberOfSectors = %ld\n",
                         (unsigned long)( (SHCI_C2_NvmStartErase_Evt_t*)p_sys_event->payload )
                             ->NumberOfSectors );
            break;
...
#    else
    ...
    // HAL_GPIO_WritePin( GPIOD, BLE_LED_Pin, GPIO_PIN_SET);
...
void HAL_GPIO_EXTI_Callback( uint16_t GPIO_Pin )
{
    switch ( GPIO_Pin )
    {
        ...
        case HF_STIM_SYNC_Pin:

            __HAL_GPIO_EXTI_CLEAR_IT( GPIO_PIN_8 );
            HAL_NVIC_ClearPendingIRQ( EXTI9_5_IRQn );
            tx_thread_resume( &threadRampStim );
