...
extern uint8_t mapping[] __attribute__((aligned(4)));
extern unsigned int ARG7;
...
unsigned int SATURN_READ_SINGLE (unsigned int address);
...
void Min_Amplitude_Ramp(CommandData_T* cmd);
void Max_Amplitude_Ramp(CommandData_T* cmd);
void Duty_Cycle(CommandData_T* cmd);
void Rep_Freq(CommandData_T* cmd);
void Finite_Ramp_Duration( CommandData_T* cmd );
void Enable_Ramping (bool state);
void ramp_setup (void);
}
