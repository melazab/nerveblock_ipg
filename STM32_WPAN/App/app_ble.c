...
float a_ConnInterval[ SIZE_TAB_CONN_INT ] = { 10, 20 }; /* ms */
...
static const char a_LocalName[] = {
    AD_TYPE_COMPLETE_LOCAL_NAME, 'C', 'A', '_', 'I', 'P', 'G',
};
...
#if ( P2P_SERVER2 != 0 )
static const char a_LocalName[]
    = { AD_TYPE_COMPLETE_LOCAL_NAME, 'P', '2', 'P', 'S', 'R', 'V', '2' };
uint8_t a_ManufData[ 14 ] = {
    sizeof( a_ManufData ) - 1,
    AD_TYPE_MANUFACTURER_SPECIFIC_DATA,
    0x01,                   /*SKD version */
...
#if ( P2P_SERVER3 != 0 )
static const char a_LocalName[]
    = { AD_TYPE_COMPLETE_LOCAL_NAME, 'P', '2', 'P', 'S', 'R', 'V', '3' };
uint8_t a_ManufData[ 14 ] = {
    sizeof( a_ManufData ) - 1,
    AD_TYPE_MANUFACTURER_SPECIFIC_DATA,
    0x01,                   /*SKD version */
...
#if ( P2P_SERVER4 != 0 )
static const char a_LocalName[]
    = { AD_TYPE_COMPLETE_LOCAL_NAME, 'P', '2', 'P', 'S', 'R', 'V', '4' };
uint8_t a_ManufData[ 14 ] = {
    sizeof( a_ManufData ) - 1,
    AD_TYPE_MANUFACTURER_SPECIFIC_DATA,
    0x01,                   /*SKD version */
...
#if ( P2P_SERVER5 != 0 )
static const char a_LocalName[]
    = { AD_TYPE_COMPLETE_LOCAL_NAME, 'P', '2', 'P', 'S', 'R', 'V', '5' };
uint8_t a_ManufData[ 14 ] = {
    sizeof( a_ManufData ) - 1,
    AD_TYPE_MANUFACTURER_SPECIFIC_DATA,
    0x01,                   /*SKD version */
...
#if ( P2P_SERVER6 != 0 )
static const char a_LocalName[]
    = { AD_TYPE_COMPLETE_LOCAL_NAME, 'P', '2', 'P', 'S', 'R', 'V', '6' };
uint8_t a_ManufData[ 14 ] = {
    sizeof( a_ManufData ) - 1,
    AD_TYPE_MANUFACTURER_SPECIFIC_DATA,
    0x01,                   /*SKD version */
...
void APP_BLE_Init( TX_BYTE_POOL* p_byte_pool )
{
    ...
    SHCI_C2_Ble_Init_Cmd_Packet_t ble_init_cmd_packet = {
        ...
        { 0,             /** pBleBufferAddress not used */
         ...
         CFG_BLE_DATA_LENGTH_EXTENSION, CFG_BLE_PREPARE_WRITE_LIST_SIZE,
         CFG_BLE_MBLOCK_COUNT, CFG_BLE_MAX_ATT_MTU,
         CFG_BLE_PERIPHERAL_SCA, CFG_BLE_CENTRAL_SCA,
         CFG_BLE_LS_SOURCE, CFG_BLE_MAX_CONN_EVENT_LENGTH,
         CFG_BLE_HSE_STARTUP_TIME, CFG_BLE_VITERBI_MODE,
         CFG_BLE_OPTIONS, 0,
         CFG_BLE_MAX_COC_INITIATOR_NBR, CFG_BLE_MIN_TX_POWER,
         CFG_BLE_MAX_TX_POWER, CFG_BLE_RX_MODEL_CONFIG,
         CFG_BLE_MAX_ADV_SET_NBR, CFG_BLE_MAX_ADV_DATA_LEN,
         CFG_BLE_TX_PATH_COMPENS, CFG_BLE_RX_PATH_COMPENS,
         CFG_BLE_CORE_VERSION, CFG_BLE_OPTIONS_EXT }
    };
        ...
        case HCI_LE_META_EVT_CODE:
        {
            ...
            switch ( p_meta_evt->subevent )
            {
                ...
                case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
                {
                    ...
                    APP_DBG_MSG(
                        ">>== HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE - Connection handle: 0x%x\n",
        ...
        case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
            ...
            switch ( p_blecore_evt->ecode )
            {
                ...
                case ( ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE ):
                    ...
                    APP_DBG_MSG(
                        "     - numeric_value = %ld\n",
                        (unsigned long)( (aci_gap_numeric_comparison_value_event_rp0*)( p_blecore_evt
                                                                                            ->data ) )
                            ->Numeric_Value );
                    APP_DBG_MSG(
                        "     - Hex_value = %lx\n",
...
#    else
                    // HAL_GPIO_WritePin( BLE_LED_Port , BLE_LED_Pin, GPIO_PIN_SET);
#    endif
...
static void Ble_Hci_Gap_Gatt_Init( void )
{
    ...
    if ( ret != BLE_STATUS_SUCCESS )
    {
        APP_DBG_MSG( "  Fail   : aci_hal_write_config_data command - CONFIG_DATA_PUBADDR_OFFSET, "
                     "result: 0x%x \n",
                     ret );
    }
    else
...
static void Adv_Request( APP_BLE_ConnStatus_t NewStatus )
{
              ...
              || ( BleApplicationContext.Device_Connection_Status == APP_BLE_LP_ADV ) ) )
    {
        ...
        {
            APP_DBG_MSG(
                "==>> aci_gap_set_non_discoverable - Successfully Stopped Advertising \n" );
        }
    }
...
#else
    // HAL_GPIO_WritePin( BLE_LED_Port, BLE_LED_Pin, GPIO_PIN_RESET);
#endif
...
void ConfigureMaxThroughput( void )
{
    /* Set connection interval to minimum (7.5ms) */
    uint16_t interval_min       = 6;    /* 7.5ms (6*1.25ms) */
    uint16_t interval_max       = 6;    /* 7.5ms (6*1.25ms) */
    uint16_t slave_latency      = 0;
    uint16_t timeout_multiplier = 1000; /* 10s (1000*10ms) */
    ...
    /* Set 2M PHY for higher data rate */
    hci_le_set_phy( BleApplicationContext.BleApplicationContext_legacy.connectionHandle,
                    ALL_PHYS_PREFERENCE,
                    TX_2M_PREFERRED,
                    RX_2M_PREFERRED,
                    0 );
    ...
    hci_le_set_data_length( BleApplicationContext.BleApplicationContext_legacy.connectionHandle,
                            251,
                            2120 );
}
...
void BLE_SVC_L2CAP_Conn_Update( uint16_t ConnectionHandle )
{
    ...
    if ( mutex == 1 )
    {
        ...
        uint16_t interval_min       = CONN_P( a_ConnInterval[ 0 ] );
